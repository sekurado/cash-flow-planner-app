...
- `ReceiptOcrProvider` (domain protocol) plus platform backends extract lines: macOS Vision (`MacosVisionOcrProvider`), Windows.Media.Ocr (`WindowsOcrProvider`), and Linux Tesseract (`TesseractOcrProvider`). Other OSes use `UnsupportedReceiptOcrProvider`. `ReceiptFieldParser` maps lines to amount / date / merchant hints with per-field low-confidence flags.
...
`ReceiptOcrWorker` runs extract + parse off the UI thread. `RecordedExpensesViewModel` exposes pending path, suggestions, and confidence flags; `createExpense` attaches the pending image after a successful insert. On source runs, Settings can install platform OCR bindings (`ocr-macos`, `ocr-windows`, or `ocr-linux`) in the background when they are missing; packaged builds must include the extra at freeze time. Linux also needs a system `tesseract` binary. Cloud OCR is a Settings stub only (NFR-01: core remains offline).
...
| 11 | How should receipt photos become expense fields without sending images to the cloud? | **On-device OCR + review UI (FR-27).** macOS uses Vision via PyObjC (`ocr-macos`); Windows uses `Windows.Media.Ocr` (`ocr-windows`); Linux uses Tesseract (`ocr-linux` plus a system `tesseract` package). A pure-Python heuristic parser maps OCR lines to amount/date/merchant. The user always edits and confirms. Cloud OCR is a Settings stub, off by default, and does not upload images. See Section 8.8 and `docs/receipt-ocr.md`. |
